<template>
  ...
  >
    ...
    >
      ...
      <div class="flex items-center gap-2">
        <ClientOnly>
          ...
          <template #fallback>
            <div class="flex items-center gap-2">
              <div class="w-9 h-9 rounded-full border border-white/10 bg-white/5"></div>
    ...
    <!-- Main Content -->
    <main class="max-w-7xl mx-auto p-4 md:p-6 lg:p-8 pb-28 md:pb-8 lg:pb-8 animate-fade-in">
    ...
    <nav
      ...
      class="md:hidden fixed bottom-0 left-0 right-0 backdrop-blur-lg border-t h-[calc(4rem+env(safe-area-inset-bottom))] pb-[env(safe-area-inset-bottom)] flex items-center justify-around px-2 z-50 transition-colors duration-300"
      :class="isDark
        ? 'bg-[rgba(22,22,22,0.95)] border-white/10'
...
await useAsyncData('init-layout', async () => {
  if (user.value) {
    const profileJaCarregado = !!profile.value
...
watch(user, async (newUser, oldUser) => {
  if (newUser && (!oldUser || newUser.id !== oldUser.id)) {
    // Aguarda o perfil estar pronto (o useAuth já disparou o fetch via seu próprio watch)
    // garantindo que isAdmin está correto ANTES de buscar os campeonatos
